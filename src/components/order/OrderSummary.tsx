...
import { OrderItem, Empresa, Catalogo } from '../../types'
...
export function OrderSummary({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // 3. Send email notification (non-blocking — failure doesn't cancel the order)
      try {
        await sendOrderEmail({ empresa, items, observaciones: observaciones.trim(), pedidoId: pedido.id })
      } catch (emailErr) {
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          <div className="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-primary/5 to-secondary/5">
            <h1 className="text-2xl font-bold text-text">Resumen del pedido</h1>
            <p className="text-slate-500 text-sm mt-1">
              {empresa.nombre} · {totalItems} {totalItems === 1 ? 'artículo' : 'artículos'}
            </p>
          </div>
